...
// -- i think this one is the better one? just use which ever one fits lol -- //

// "Partline" comes from the old helper name, when it used to be just badelines!
[CustomEntity(new string[] { "Partline/UBadelineDummy" })]
[Tracked]
public class UniversalBadelineDummy : Entity
{
    ...
    public static UniversalBadelineDummy FindFirst(string variant = "blue")
    {
        foreach (var i in Engine.Scene.Entities.FindAll<UniversalBadelineDummy>())
        {
            if (i.variant == variant) return i as UniversalBadelineDummy;
        }
        return null;
    }
    public static List<UniversalBadelineDummy> FindAll(string variant = "blue")
    {
        List<UniversalBadelineDummy> l = new();
        foreach (var i in Engine.Scene.Entities.FindAll<UniversalBadelineDummy>())
        {
    ...
    public static UniversalBadelineDummy GetFirstByVariant(BadelineVariants variant)
    {
        //BadelineVariants color = new BadelineVariants();
        foreach (var i in Engine.Scene.Entities.FindAll<UniversalBadelineDummy>())
        {
            if (Enum.TryParse(i.variant, true, out BadelineVariants color))
            {
                if (color == variant) return i;
            }
        }
        return null;
    }

    public static UniversalBadelineDummy[] GetByVariant(BadelineVariants variant)
    {
        List<UniversalBadelineDummy> list = new List<UniversalBadelineDummy>();
        foreach (var i in Engine.Scene.Entities.FindAll<UniversalBadelineDummy>())
        { 
            if (Enum.TryParse(i.variant, true, out BadelineVariants color))
            {
                if (color == variant) list.Add(i);
            }
        }
        return list.ToArray();
    }
    ...
    public UniversalBadelineDummy(Vector2 position, string variant = "blue", bool left = false, bool floating = true, string animation = "", string flag = "", bool ifset = false, bool soul = false)
        : base(position)
    {
        this.flag = flag;
        this.ifset = ifset;
    ...
    public UniversalBadelineDummy(EntityData data, Vector2 offset)
    ...
    public override void Update()
    {
        ...
        {
            if (Sprite.CurrentAnimationID != null && Sprite.CurrentAnimationID != "" && Sprite.CurrentAnimationID != "sitPlay" && Sprite.CurrentAnimationID != "sitPlayA" && Sprite.CurrentAnimationID != "sitPlayB" && Sprite.CurrentAnimationID != "soul")
                //if (Spritealt.animations.ContainsKey(Sprite.currentAnimation.ToString()))
                Spritealt.Play(Sprite.CurrentAnimationID, false, false);
            else if (Sprite.CurrentAnimationID == "sitPlay" || Sprite.CurrentAnimationID == "sitPlayA" || Sprite.CurrentAnimationID == "sitPlayB")
                Spritealt.Play("pretendDead");
        }
    ...
    public void ToSoul()
    {
        ...
        if (Enum.TryParse(variant, true, out color))
        {
            Sprite.Color = BadelineHairColors.UnderTaleColors[(int)color].ToColor();
        }
    }
}
