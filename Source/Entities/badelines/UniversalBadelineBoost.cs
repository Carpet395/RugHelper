...
namespace Celeste.Mod.Partline.Celeste.Mod.Partline.Entities;

[CustomEntity(new string[] { "Partline/UBadelineBoost" })]
public class UniversalBadelineBoost : Entity
{
    ...
    private BadelineVariants color = new BadelineVariants();
    ...
    public UniversalBadelineBoost(Vector2[] nodes, bool lockCamera, bool canSkip = false, bool finalCh9Boost = false, bool finalCh9GoldenBoost = false, bool finalCh9Dialog = false, string variant = "blue")
        : base(nodes[0])
    {
        base.Depth = -1000000;
        this.nodes = nodes;
        ...
        color = new BadelineVariants();
        if (Enum.TryParse(variant,true,out color)) {
            Add(sprite = GFX.SpriteBank.Create($"{BadelineHairColors.Names[(int)color]}Boost"));
            Add(stretch = new Image(GFX.Game[$"objects/{BadelineHairColors.Names[(int)color]}boost/stretch"]));
    ...
    public UniversalBadelineBoost(EntityData data, Vector2 offset)
        : this(data.NodesWithPosition(offset), data.Bool("lockCamera", defaultValue: true), data.Bool("canSkip"), data.Bool("finalCh9Boost"), data.Bool("finalCh9GoldenBoost"), data.Bool("finalCh9Dialog"), data.Attr("variant"))
    {
    }
    ...
    private IEnumerator BoostRoutine(Player player)
    {
        ...
        UniversalBadelineDummy badeline = new UniversalBadelineDummy(Position, variant);
    ...
    {
        if (sprite.Visible && base.Scene.OnInterval(0.05f) && P_Ambience != null)
        {
