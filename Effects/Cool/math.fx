...
float4 SpritePixelShader(float2 uv : TEXCOORD0) : COLOR0
{
    ...
    float2 worldPos = floor(((uv * Dimensions) + CamPos ));
    float num = 280.0 * sizeMult * 15;
    float value = (worldPos.x + - (worldPos.y / 1.5) + Time * 50.0 * speedMult / 15) % num / num;
    return (float4(HsvToColor(0.4 + YoYo(value) * 0.4, 0.6, 0.5)) * 1) * color;
